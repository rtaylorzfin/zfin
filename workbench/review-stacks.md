...
That used to mean one `hostctl` entry per stack. Those are now **removed**, and resolution
is left to DNS -- but calling wildcard DNS a *prerequisite* overstated it. It is the
arrangement this design works best under, not one anyone committed to: on
is-zfin-dkr-p02, `*.coral.zfin.org` does NOT resolve (observed 2026-09-22 --
`bogusslug.coral.zfin.org` is NXDOMAIN while real stack names resolve), so records are
being created per stack after all, or resolved client-side. Where a wildcard does exist
there is one record per zone instead of one per feature and teardown has nothing to undo;
where it does not, the per-feature step is back and nothing in the tooling says so. `z feature new --review` warns
...
- **Name resolution is unowned.** The tooling assumes a stack's hostname resolves and never
  checks it. Three arrangements are in use or possible -- a zone wildcard in real DNS
  (nothing per feature), a record per stack (a manual step `z feature new` does not mention),
  or client-side dnsmasq on each developer's machine (works, but only for that developer, so
  a shared review URL is not actually shareable). Today `is-zfin-dkr-p02` has no zone
  wildcard, so it is one of the latter two, decided per person rather than by design.

  Two cheap improvements regardless of which is chosen: have `z feature new` resolve the
  hostname it just created and say so when it does not, and print the record to add when it
  cannot. A stack that provisions cleanly and then cannot be reached by name is the failure
  this would catch, and it currently surfaces as a browser error minutes later.
