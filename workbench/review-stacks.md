...
A new overlay `docker/docker-compose.review-member.yml` (composed *after* the
...
`docker-compose.review-upstream.yml` sets `VIRTUAL_PROTO: https` / `VIRTUAL_PORT: 443`
...
**`required: false` is what makes shared-db stacks survive it.** `docker-compose.shared-db.yml`
...
`docker-compose.shared-db.yml` profile-suppresses them, and the real data lives in the
...
Files: `docker/docker-compose.review.yml`, `docker/docker-compose.review-member.yml`,
...
- **Collapsing the proxy chain.** Today a request crosses `ngproxy` → `review-proxy` →
  ...
  *`ngproxy` → `review-proxy` is genuinely redundant.* Both are the same nginx-proxy
  ...
  one holding the host's `:443` — which is why `docker-compose.review-upstream.yml`
