-- ============================================================================
-- Regenerate-Chromosome-Mart_d — additive layer over Refresh-GBrowse-Tracks_d
-- ============================================================================
-- This script runs downstream of Refresh-GBrowse-Tracks_d (configured via
-- hudson.tasks.BuildTrigger in that job's config.xml). The daily refresh
-- handles the bulk DELETE+INSERT of UCSC, Ensembl, Zfin, and DirectSubmission
-- rows in sequence_feature_chromosome_location_generated. This script adds
-- only the pieces the refresh path doesn't cover:
--
--   §E. ENSDARG-fallback EnsemblStartEndLoader rows (genes with an ENSDARG
--       accession but no ENSDART transcript link).
--   §H. zmp gbrowse-track tag for ZDB-PUB-250905-18 (refresh §H tags
--       ZDB-PUB-130425-4).
--
-- Section markers §A–§K below correspond to the same-named sections in
-- server_apps/data_transfer/Ensembl/updateSequenceFeatureChromosomeLocation.sql
-- (the refresh script). Sections marked "REDUNDANT — handled by refresh" are
...
--     Bulk DELETE of the five sources we'd otherwise own. The refresh runs
--     this immediately before re-inserting fresh rows, and we run downstream
--     of the refresh, so by the time this script starts the table is already
--     in its post-refresh state.
-- ----------------------------------------------------------------------------
...
--     A follow-up PR may simplify this once those sections are deleted.
-- ----------------------------------------------------------------------------
...
--     Note: the UCSC INSERT here was already disabled before this branch
--     (commit 1c246db2, ZFIN-9989) because the refresh path was the
--     canonical writer; this comment-out makes that explicit.
--     The pruneUcscLinks.sh post-step in regenChromosomeMart.sh removes any
--     UCSC rows whose accession isn't in UCSC's danRer11 refGene track.
-- ----------------------------------------------------------------------------
...
--     Warehouse-unique; no equivalent in refresh.sql. Introduced by
--     ZFIN-9989 (PR #1619) for genes whose ENSDARG accession has no ENSDART
--     transcript link (so the refresh §D ENSDART path produces nothing for
--     them).
--
--     The NOT EXISTS clause makes this idempotent — subsequent runs skip
--     rows already inserted. We can't rely on uk_sfclg_unique_location +
--     ON CONFLICT here because that constraint covers sfclg_assembly and
--     sfclg_location_subsource, which we leave NULL; PostgreSQL treats
--     NULLs in unique indexes as distinct, so the constraint doesn't fire
--     on rerun.
-- ----------------------------------------------------------------------------
insert into sequence_feature_chromosome_location_generated (sfclg_data_Zdb_id,
       	    			       sfclg_chromosome,
...
select distinct dblink_linked_recid,
  ...
  -- don't include fdb_db_display_name = 'ExpressionAtlas'
  and fdb_db_pk_id != 91
  -- skip rows we've already inserted (idempotence)
  and not exists (
...
-- ----------------------------------------------------------------------------
-- §H. ACTIVE (partial) — zmp gbrowse-track tag for ZDB-PUB-250905-18.
--     Refresh §H tags ZDB-PUB-130425-4; this script tags the additional pub.
--     The WHERE guard makes it idempotent.
-- ----------------------------------------------------------------------------
...
--     Cleanup deletes for AB / U / 0 chromosomes across the same source set.
-- ----------------------------------------------------------------------------
-- delete from sequence_feature_chromosome_location_generated
