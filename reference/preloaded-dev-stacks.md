...
per checkout, one `.zenv` at its root — nothing to search for):
...
z feature new ZFIN-1234                       # worktree, .env, .zenv, IP, /etc/hosts, boot, deploy
...
#    after the slug, sitting in the worktree with .zenv/activate sourced. (This is the
#    one step z can't do for your current shell: `cd` and `source` change the CALLING
#    shell, which a child process cannot reach. Hence the session.)
zrun -c "gradle dirtydeploy"                  # deploy this branch's delta (warm app)
# ...edit / dirtydeploy loop...

# ...without --tmux, enter it the old way:
cd ../wt-zfin-1234
...
predates the stack, i.e. one provisioned with `--existing-branch`), `hostctl remove <slug>`, and
