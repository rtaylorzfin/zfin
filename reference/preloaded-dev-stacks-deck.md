...
> **SUPERSEDED — do not present.** This deck describes the system before seeds replaced the
> preloaded images, before the review proxy, and while `.zenv` activation still existed. Its
> commands (`zrun`, `source .zenv/activate`, `z feature build-preloaded`) no longer exist.
> Kept as a record of the earlier design; see **review-stacks-deck.md** for the current one,
> and **dev-stacks.md** for the written guide.
...
├── docker-compose.overlay-shared.yml     PROVIDER: run the shared zfin_shared db+solr
└── docker-compose.overlay-shared-db.yml  CONSUMER: suppress local db/solr, share zfin_shared
```
...
- **`reference/dev-stacks.md`** — the full written guide
