...
│   └── zfin-10475/
├── review/                      review-proxy state: the wildcard cert and the IP allowlist
│   ├── certs/
│   └── vhost.d/
...
seeds are captured once. If a host carries several checkouts (`coral`, `cell`), they should
share one `ZFIN_DEV_ROOT` so they share that state.
