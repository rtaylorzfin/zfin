...
| `docker-compose.overlay-review-upstream.yml` | that proxy runs behind another nginx-proxy |
| `docker-compose.overlay-review-member.yml` | this stack is routed through the proxy |
...
- **Memory:** each feature's solr is capped lean (`4g` heap / `6g` limit) by
  `docker-compose.overlay-review-member.yml`; the base stack keeps prod sizing (`12g`/`16g`). Running
