...
class ZfinUtil {
    ...
    /** True when this host's review proxy is the ONLY nginx-proxy on it -- i.e. it has replaced
     *  whatever used to own :80/:443 rather than sitting behind it. Set ZFIN_REVIEW_SOLE=true in
     *  docker/.env. A host-level fact, not a per-command flag: the proxy image, the vhost key a
     *  stack advertises, and what `z feature refresh` writes all have to agree, and a flag on one
     *  command could not keep them in step. */
    boolean reviewSole() { env('ZFIN_REVIEW_SOLE', '').toLowerCase() in ['1', 'true', 'yes'] }
