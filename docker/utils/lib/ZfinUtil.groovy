...
class ZfinUtil {
    ...
    List<Map> featureStacks() {
        ...
        wts.collect { wt ->
            ...
            [ slug    : wt.name.replaceFirst('^wt-', ''),
              ...
              worktree: wt.name,
              data    : spec?.data ?: 'own',
              state   : running.contains(proj) ? 'up'
    ...
    File archiveClaudeSession(String project, String slug) {
        ...
        def out = new File(new File(archiveDir(), 'sessions'),
                           "${slug}-${new Date().format('yyyyMMdd-HHmmss')}.tgz")
    ...
    Map writeSeedManifest(File dir, Map meta) {
        ...
        def m = [tag: meta.tag, created: new Date().format('yyyy-MM-dd HH:mm:ss'),
                 ...
                 pg_major: meta.pg_major, volumes: vols]
