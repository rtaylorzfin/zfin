...
class BuildPreloaded {
    def run(List args, ZfinUtil zfinUtil) {
        ...
        [pgVol, solrVol].each { v -> if (runQuietly(['docker', 'volume', 'inspect', v]) != 0) die("volume '$v' not found -- is project '$project' loaded? (try --project)")
        }
        ...
        def requireVols = { List vns, String flag, String hint ->
            vns.each { vn ->
                def v = "${project}_${vn}"
                if (runQuietly(['docker', 'volume', 'inspect', v]) != 0) die("$flag: volume '$v' not found -- $hint")
            }
        }
...
// carry real data and must never be pushed (see header).
        ...
        if (!keep) {
            pgTarball.delete(); solrTarball.delete()
        }
