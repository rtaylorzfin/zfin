...
//   z review up      [--acme]      also run acme-companion, so THIS proxy issues the
//                                  certificates. Only when it owns :80/:443 -- contradicts
//                                  --upstream. Needs ZFIN_REVIEW_ACME_* set from the host's
//                                  existing issuer (CA URI, EAB key/kid, email).
...
//   ZFIN_REVIEW_SOLE           true when this proxy is the ONLY nginx-proxy on the host, i.e.
//                              it has REPLACED the one that used to own :80/:443. Then it
//                              discovers by the stock VIRTUAL_HOST -- so instances are served
//                              too, not just feature stacks -- and `z feature new` writes
//                              DOCKER_EXTERNAL_VHOST rather than DOCKER_REVIEW_VHOST. Run
//                              `z feature refresh --all` after changing it.
...
class ReviewStack {
    def run(List args, ZfinUtil zfinUtil) {
        ...
        def bindArg = null; def force = false; def upstream = false; def acme = false
        for (int i = 0; i < rest.size(); i++) {
            switch (rest[i]) {
                case '--bind':  bindArg = rest[++i]; break
                case '--upstream': upstream = true; break
                // Issue real certificates from this proxy. Only meaningful when it OWNS :80/:443
                // -- behind an upstream, that proxy is already the one terminating TLS and
                // running its own issuer, and a second one would compete for the same names.
                case '--acme': acme = true; break
        ...
        def httpPort  = httpBind.substring(httpBind.lastIndexOf(':') + 1)
        ...
        // The fallback page ships with the repo and is bound over the html volume.
        zfinUtil.childEnv['ZFIN_REVIEW_FALLBACK_SRC'] = new File(DOCKER, 'review-proxy').absolutePath
        def sole = zfinUtil.reviewSole()
        zfinUtil.childEnv['ZFIN_REVIEW_DISCOVERY_ENV'] = sole ? 'VIRTUAL_HOST' : StackConfig.REVIEW_VHOST_ENV
        ...
        switch (sub) {
            case 'up':
                // Did you mean --upstream? Two variables exist ONLY for that mode -- the
                // upstream overlay refuses to render without ZFIN_REVIEW_UPSTREAM_NET, and
                // ZFIN_REVIEW_TRUSTED_PROXY is meaningless unless something fronts us. Having
                // configured either and then run standalone is a mistake every time, and the
                // symptom otherwise is a port-bind failure from the daemon that says nothing
                // about modes. Said BEFORE the port check, because the ports might happen to be
                // free and the stack would then come up in the wrong shape.
                if (!upstream) {
                    def why = []
                    if (zfinUtil.env('ZFIN_REVIEW_UPSTREAM_NET', '')) why << 'ZFIN_REVIEW_UPSTREAM_NET'
                    if (zfinUtil.env('ZFIN_REVIEW_TRUSTED_PROXY', '')) why << 'ZFIN_REVIEW_TRUSTED_PROXY'
                    if (why) {
                        System.err.println("!! ${why.join(' and ')} ${why.size() > 1 ? 'are' : 'is'} set, " +
                                           "but this is a STANDALONE run.")
                        System.err.println("   ${why.size() > 1 ? 'Those variables only mean' : 'That variable only means'}" +
                                           " anything behind another proxy. Standalone binds :${httpPort}")
                        System.err.println("   and :${httpsPort} directly, which a host already running one cannot spare.")
                        System.err.println("   Did you mean:  z review up --upstream")
                    }
                }

                // BOTH ports. Checking only :443 missed the case that actually happens on a
                // shared VM: an upstream proxy owns :80 as well, and the run died on a raw
                // "Bind for 0.0.0.0:80 failed: port is already allocated" from the daemon.
                def clash = upstream ? [:] : [httpPort, httpsPort]
                        .collectEntries { p -> [(p): portConflicts(p)] }.findAll { p, c -> c }
                if (clash && bind == '0.0.0.0') {
                    die("port(s) already published: " +
                        clash.collect { p, c -> "${p} by ${c.join(', ')}" }.join('; ') + "\n" +
                        "   A 0.0.0.0 bind cannot coexist with another publisher of the same port.\n" +
                        "   If this host already runs a proxy on :80/:443 -- the shared VMs do --\n" +
                        "   run BEHIND it rather than beside it:\n" +
                        "     z review up --upstream\n" +
                        "   Otherwise free the ports (z stop httpd, in each of those stacks), or\n" +
                ...
                info(sole ? "mode: SOLE -- the only nginx-proxy here; discovers by VIRTUAL_HOST, so instances are served too"
                          : "mode: coexisting -- discovers by ${StackConfig.REVIEW_VHOST_ENV}; instances on VIRTUAL_HOST are NOT ours")
                if (sole && upstream) die("ZFIN_REVIEW_SOLE is set but this is an --upstream run.\n" +
                        "   Sole means this proxy replaced the host's other one; --upstream means it\n" +
                        "   sits behind it. Unset one.", 2)
                ...
                // Two kinds of leftover, both of which beat --remove-orphans to the punch:
                //   1. our container_name held by a container under an OLD service label (the
                //      ngproxy -> proxy rename), and
                //   2. a container of a service this project no longer HAS -- `landing`, deleted
                //      when the fallback page moved into the proxy. That one keeps publishing
                //      :80, so the new proxy fails to bind while compose is still getting round
                //      to sweeping it.
                def liveServices = ['proxy'] as Set
                captureOutput(['docker', 'ps', '-aq', '--filter',
                        "label=com.docker.compose.project=${StackConfig.REVIEW_PROJECT}"])
                    .readLines().findAll { it }.each { id ->
                        def svc = captureOutput(['docker', 'inspect', id, '--format',
                                '{{index .Config.Labels "com.docker.compose.service"}}'])?.trim()
                        def nm  = captureOutput(['docker', 'inspect', id, '--format',
                                '{{.Name}}'])?.trim()?.replaceFirst('^/', '')
                        if (svc && !(svc in liveServices)) {
                            info("removing leftover '${nm}' (service '${svc}' is not in this project any more)")
                            runQuietly(['docker', 'rm', '-f', id])
                        }
                    }
                ...
                if (acme && upstream) die("--acme and --upstream are contradictory.\n" +
                        "   Behind an upstream proxy, THAT proxy terminates TLS and runs its own\n" +
                        "   issuer; a second one would compete for the same names. --acme is for\n" +
                        "   when this proxy owns :80/:443 instead of sitting behind one.", 2)
                runCommand(compose + (acme ? ['--profile', 'review-acme'] : []) +
