...
//        zbuild all -> [build-preloaded + new-feature]
//
// --dry-run runs the checks and prints the plan (using defaults, no prompts) without
// executing the heavy build/load steps.
//
// PREREQUISITE -- WILDCARD DNS FOR THE REVIEW ZONE
// Feature stacks are served through the shared review proxy at <slug>.<zone>, and there are
// no per-stack /etc/hosts entries any more: one wildcard record resolves every stack that
// will ever exist, so nothing is added per feature and nothing needs cleaning up at teardown.
// On a SHARED REVIEW HOST the right answer is a real wildcard A record (*.<zone> -> that
// host). Nothing is then needed per developer, and the URL is a link anyone can open --
// which is the whole point of a review stack.
//
// On a LAPTOP, dnsmasq plus the per-domain resolver (macOS shown):
//
//   brew install dnsmasq
//   echo 'address=/review.zfin.test/127.0.0.1' >> $(brew --prefix)/etc/dnsmasq.conf
//   sudo brew services start dnsmasq
//   sudo mkdir -p /etc/resolver
//   echo 'nameserver 127.0.0.1' | sudo tee /etc/resolver/review.zfin.test
//
// Verify with `ping anything.review.zfin.test` -- NOT dig or nslookup, which query DNS
// directly and bypass /etc/resolver, so they report failure while every browser works.
// On Linux the dnsmasq config is the same idea, but the resolver wiring differs
// (systemd-resolved wants its stub disabled, or a per-domain DNS= entry).
//
// Either way there is a fallback needing no DNS at all: every routed stack resolves to the
// SAME address, so one /etc/hosts line per stack works, and `z feature new` prints it.
// That command warns when the name would not resolve, rather than provisioning a stack that
...
// --- 5. the plan (everything routes through the single front door `z`) -----------
        ...
        if (firstTicket) {
            ...
            plan << [zExe, 'feature', 'new', firstTicket, '--tag', tag, '--up']
        }
