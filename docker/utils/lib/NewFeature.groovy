...
// defaults instead, so an unattended caller never blocks on stdin.
        ...
        if (interactive) {
            ...
            if (!('tag' in explicit)) {
                def tIn = con.readLine("  seed [${tag ?: 'none captured yet'}]: ")?.trim()
                if (tIn) tag = tIn
            }
        ...
        info(tag ? "seed: $tag" : "seed: none (--shared-db shares the zfin_shared data tier)")
...
// doesn't need a seed -- it needs the shared data stack up (its external network).
        ...
        if (doSharedDb) {
            ...
            if (!tag) die("no seed to restore from.\n" +
                    "   Capture one from a loaded stack:  z seed create --from <project>\n" +
                    "   ...or share a db+solr copy instead: z feature new <ticket> --shared-db")
...
// rm keeps the branch instead.
        ...
        StackConfig.featureEnv(host).each { k, v -> outEnv << "$k=$v\n" }
...
//     stale, and nothing to refresh after the tooling changes.
        ...
        def dataOverlay = doSharedDb ? 'docker-compose.shared-db.yml' : null
        ...
        def overlays = ([dataOverlay] - null) + ['docker-compose.review-member.yml']
...
// container startup + writing many small files, which parallelism overlaps.)
        ...
        def restore = { List vns ->
            ...
            results.each { r ->
                volSecs[r.vn] = [secs: r.secs, mb: r.mb]
        ...
        def dataVols = doSharedDb ? [] : StackConfig.DATA_VOLS
        def toRestore = dataVols + (warmApp ? appVols : []) + (warmCaches ? cachesPresent : [])
